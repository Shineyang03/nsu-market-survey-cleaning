"""How many price points (rungs) does each prov x mun x item x nsu case have?

Reads NSU_prices_from_Makayla.csv and reports the rung availability that Limit 2
...
labelled "unique municipal price only", and a rung() helper written as
...
exactly two price points" and a 67.5% single-rung share. The real figures are
below. Count distinct price LEVELS and enumerate label COMBINATIONS -- never
first-match-wins over a set.

Definitions used:
  rung  = a distinct price level that can be paired with a size.
          - a full mp25/mp50/mp75 triple gives 3 rungs
          - the observed municipal price level(s) give 1-2 rungs
          - a lone municipality or province median gives 1 rung
  The province median accompanying a municipal price is treated as a FALLBACK
  REFERENCE, not a second rung: the two are estimates of the same central
  tendency at different geographies, so pairing them as small/large would be
  meaningless. The alternative reading (count every distinct level as a rung) is
...
def rungs_reference_reading(rows):
    """Municipal price = rung; an accompanying province median = reference."""
...
def rungs_level_reading(rows):
    """Every distinct price level counts as a rung."""
...
def show(title, counter, total):
    ...
    for k in sorted(counter):
        print(f"  {k} rung(s): {counter[k]:>5}  {100 * counter[k] / total:>5.1f}%")
...
def main():
    ...
    show("rung count -- province median as REFERENCE (preferred)",
         collections.Counter(rungs_reference_reading(r) for r in cells.values()), total)
    show("rung count -- every distinct price level a rung (alternative)",
         collections.Counter(rungs_level_reading(r) for r in cells.values()), total)
