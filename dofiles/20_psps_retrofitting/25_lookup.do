...
* ---- n_g arrived from all three branches ---------------------------------------------
* THIS ASSERTION IS THE POINT OF THE `keep' LISTS ABOVE. `append' fills a column absent
* from one of the appended files with MISSING, silently -- so dropping n_g from one
* branch's keep list would not error, and every row of that branch would arrive with no
* evidence count. d_thin would then be missing there, which reads as "no value published"
* about rows that carry one.
*
* So: wherever there is a usable weight, there must be a count of the weighings behind it.
assert !missing(n_g) if d_point_usable == 1

* And the converse. A row with no weighings behind it -- Branch S's unusable points --
* must NOT carry a zero, which would read as a real count of none.
assert missing(n_g) if d_point_usable == 0

* Read per branch, because that is the cut at which a column lost in a `keep' would show
* up as a suspiciously clean zero.
di as res _n "weighings behind each published point, by branch:"
table branch, statistic(frequency) statistic(mean n_g) nformat(%9.2f)
...
* n_g must survive into this variant too, and for a sharper reason than symmetry: #11
* compares the household grams the two lookups produce, and a comparison in which one
* side carries the evidence count and the other does not would attribute a difference in
