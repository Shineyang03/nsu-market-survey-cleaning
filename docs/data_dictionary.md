...
| `d_point_usable`, `unusable_why` | Whether the price point can carry a conversion, and if not, why. 466 of 3,638 rows are unusable and say so rather than being dropped. |
...
| `d_thin` | 1 = fewer than `THIN = 3` weighings stand behind this row's value. **A warning, not a filter:** the row is published either way and is never rerouted to a pooled weight for being thin (#31, decided 2026-09-16; A3). Read off whichever count that file publishes — `n_g` in the lookups, `n_g_used` in `psps_grams`, which is the rung *actually used* and so differs from the matched point's count on a fallback row. **Missing, not zero, where nothing is published** — an unusable price point, an unconverted household, a standard-unit row. Zero throughout both hetero-blind files by construction, since that ladder is gated at every rung. |
